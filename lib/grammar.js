...
Grammar = (function() {
  ...
  Grammar.prototype.headTail = function(head, tail) {
  ...
  Grammar.prototype.inlineConstraint = function(prop, op, rest) {
    ...
    command = this.parser.parse("&[" + prop + "] " + opMap[op] + " " + rest).commands[0];
    this._addCommand(command);
  ...
  Grammar.prototype.inlineSet = function(prop, rest) {
    prop = prop.join('').trim();
    rest = rest.join('').trim();
    this._addCommand(['set', prop, rest]);
    return "inline set";
  };
  ...
  Grammar.prototype.forEach = function(type, selector, javaScript) {
    return this._addCommand([type, selector, javaScript]);
  };
