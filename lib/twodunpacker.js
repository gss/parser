var analyze, expand2dProperties, propertyMapping, _addConstraintForUnpacking, _analyze, _analyzeTailAstFor2DProperty, _changePropertyName, _clone, _removeTempState, _routeTraversalFor2DExpansion, _traverseAstFor2DProperties, _unpackRuleset2dConstraints, _unpackStay2dConstraint,
  _this = this;
...
_analyze = function(node, commands, firstLevelCmd, buffer) {
  ...
  if (node.length >= 3) {
    ...
    if (commandName === 'rule') {
      ...
      if (!node._has2dProperty && (headNode._has2dProperty || tailNode._has2dProperty)) {
